#include "cmsis_os.h"
#include "adc.h"
#include "tasks_init.h"
#include "adc_task.h"
#include "temperature_task.h"
#include "pressure_task.h"
#include "board.h"
#include "log.h"

osThreadId   adc_task_hdl;


typedef struct
{
    uint32_t cusum;
    uint16_t average;
    uint16_t cnt;
    uint16_t err;
}adc_sample_t;

static volatile uint16_t sample[2];
static adc_sample_t temperature;


void  HAL_ADC_ConvCpltCallback(ADC_HandleTypeDef *hadc)
{
    osSignalSet(adc_task_hdl,ADC_TASK_SIGNAL_ADC_COMPLETED);   
}


void  HAL_ADC_ErrorCallback(ADC_HandleTypeDef *hadc)
{
    osSignalSet(adc_task_hdl,ADC_TASK_SIGNAL_ADC_ERROR);   
}


static int adc_start()
{
    HAL_StatusTypeDef status;
    status =HAL_ADC_Start_DMA(&hadc1,(uint32_t*)sample,2);
    if (status != HAL_OK) {
        log_error("start adc dma error:%d\r\n",status);
        return -1;
    }
    return 0;
}

static int adc_stop()
{
    HAL_StatusTypeDef status;
    status = HAL_ADC_Stop_DMA(&hadc1);
    if (status != HAL_OK) {
        log_error("stop adc dma error:%d\r\n",status);
        return -1;
    }
    return 0;
}


static void adc_calibration()
{
    HAL_ADCEx_Calibration_Start(&hadc1);
}

static void adc_reset()
{
    HAL_ADC_MspDeInit(&hadc1);
    MX_ADC1_Init();
    adc_stop();
    adc_calibration();
    adc_stop();
}
void adc_task(void const * argument)
{
    osEvent signals;
    temperature_task_message_t temperature_msg;
    int  rc;
 
    adc_stop();
    adc_calibration();
    adc_stop();
    while (1) {
    osDelay(ADC_TASK_INTERVAL);
    rc = adc_start();
    if (rc != 0 ) {
        adc_reset();
        continue;
    } 
  
    signals = osSignalWait(ADC_TASK_SIGNALS_ALL,ADC_TASK_ADC_TIMEOUT);
    if (signals.status == osEventSignal) {
        if (signals.value.signals & ADC_TASK_SIGNAL_ADC_COMPLETED) { 
                /*温度采样值计算*/    
            if (temperature.cnt < ADC_TASK_ADC_SAMPLE_MAX) {
                /*采样值异常处理*/
                if (sample[ADC_TASK_TEMPERATURE_IDX] <= ADC_TASK_ADC_VALUE_MIN ||\
                    sample[ADC_TASK_TEMPERATURE_IDX] >= ADC_TASK_ADC_VALUE_MAX) { 
                    temperature.cusum = 0;
                    temperature.cnt = 0;
                    temperature.err ++; 
                    if (temperature.err >= ADC_TASK_ADC_ERR_MAX) {
                        log_error("temperature sample error.\r\n");
                        temperature.err = 0;
                        /*构建采样错误消息*/
                        temperature_msg.head.id = TEMPERATURE_TASK_MSG_TYPE_ADC_ERR; 
                        log_assert_bool_false(xQueueSend(temperature_task_msg_q_id,&temperature_msg,ADC_TASK_PUT_MSG_TIMEOUT) == pdPASS);
                    }
                } else {
                    temperature.cusum += sample[ADC_TASK_TEMPERATURE_IDX];   
                    temperature.cnt ++;
                    temperature.err = 0;
                }
            } else {
                temperature.average = temperature.cusum / temperature.cnt;  
                /*总和清零 等待取样*/
                temperature.cusum = 0;
                temperature.cnt = 0;
                /*构建采样消息*/
                temperature_msg.head.id = TEMPERATURE_TASK_MSG_TYPE_ADC_VALUE; 
                temperature_msg.content.adc = temperature.average;
                log_assert_bool_false(xQueueSend(temperature_task_msg_q_id,&temperature_msg,ADC_TASK_PUT_MSG_TIMEOUT) == pdPASS);
            }
        }
    }
    }
 
}

